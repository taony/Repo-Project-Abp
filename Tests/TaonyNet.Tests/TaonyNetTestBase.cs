...
namespace TaonyNet.Tests
{
    public abstract class TaonyNetTestBase : AbpIntegratedTestBase<TaonyNetTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaonyNetDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaonyNetDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaonyNetDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaonyNetDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaonyNetDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaonyNetDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaonyNetDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaonyNetDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaonyNetDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaonyNetDbContext, Task<T>> func)
        {
            T result;
